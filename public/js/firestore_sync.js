...
/* ═══════════════════════════════════════════════
   Realtime sync (onSnapshot listeners)
═══════════════════════════════════════════════ */
//
// After syncNotesOnLogin does the initial backfill, these listeners stream
// every subsequent Firestore change (add/modify/remove) into IndexedDB so
// any device sees what every other device just did, within ~1 second.
//
// Why this matters: deletes done in browser A used to live in browser B's
// IDB forever (delete-tracking is per-browser localStorage). With realtime
// listeners, browser B receives a 'removed' event the moment A deletes the
// doc on Firestore — IDB stays consistent without any heuristic cleanup.
//
// Loop safety: Firestore's onSnapshot does NOT echo back writes that the
// same client just made (it tracks pendingWrites). Combined with the
// existing ghost guard in saveNote, an empty doc cannot enter IDB — even
// from a Firestore event.

let _notesUnsubscribe = null;
let _foldersUnsubscribe = null;
let _renderDebounceHandle = null;

function _scheduleHomeRender() {
  // Coalesce rapid-fire Firestore events into a single home render.
  // 150ms is short enough to feel instant but long enough to batch a
  // multi-doc change (e.g. folder reorder writing many sortOrder updates).
  if (_renderDebounceHandle) clearTimeout(_renderDebounceHandle);
  _renderDebounceHandle = setTimeout(() => {
    _renderDebounceHandle = null;
    if (typeof renderHomeView === 'function') {
      try { renderHomeView(); } catch (e) { console.warn('[realtime] renderHomeView failed:', e); }
    }
  }, 150);
}
...
function startRealtimeSync() {
  if (!currentUser) return;
  stopRealtimeSync(); // idempotent — clear any previous listeners first

  const notesRef = userNotesRef();
  const foldersRef = userFoldersRef();
  if (!notesRef || !foldersRef) return;

  // Notes listener — every Firestore add/modify/remove flows into IDB.
  _notesUnsubscribe = notesRef.onSnapshot(async (snap) => {
    // Skip the initial snapshot's batch of "added" events — syncNotesOnLogin
    // already populated IDB from the same data. After that, every new event
    // is a real remote change to apply locally.
    const isInitial = snap.metadata.fromCache && snap.metadata.hasPendingWrites === false
                      && _notesInitialDelivered === false;
    const fromMyOwnWrite = snap.metadata.hasPendingWrites;

    let changedCount = 0;
    for (const change of snap.docChanges()) {
      // Don't re-apply our own writes — Firestore tracks them as pending
      // and we already updated IDB locally before pushing.
      if (change.doc.metadata.hasPendingWrites) continue;

      try {
        if (change.type === 'added' || change.type === 'modified') {
          const data = change.doc.data();
          // Skip ghosts — saveNote also guards but checking here saves IDB ops.
          const _hasTitle = data.title && data.title.trim();
          const _hasContent = (data.notesText || data.markdownContent) &&
                              (data.notesText || data.markdownContent).trim();
          if (!_hasTitle && !_hasContent) continue;

          // Hydrate fields that Firestore strips for size (notesHtml,
          // extractedImages-from-URLs) so the viewer has what it needs.
          if (!data.notesHtml && data.notesText && typeof renderMarkdown === 'function') {
            data.notesHtml = renderMarkdown(data.notesText);
          }
          if (data.slideImageUrls && !data.extractedImages) {
            data.extractedImages = data.slideImageUrls
              .map((url, i) => url ? { slideNumber: i + 1, imageBase64: url, mimeType: 'url' } : null)
              .filter(Boolean);
          }
          await saveNote(data);
          changedCount++;
        } else if (change.type === 'removed') {
          // Don't trip the deleted-notes localStorage queue — that's for
          // local→Firestore propagation. Pure IDB delete is enough here.
          try {
            const conn = await openDB();
            await new Promise((res, rej) => {
              const tx = conn.transaction('notes', 'readwrite');
              tx.objectStore('notes').delete(change.doc.id);
              tx.oncomplete = res;
              tx.onerror = e => rej(e.target.error);
            });
            changedCount++;
          } catch (e) {
            console.warn('[realtime] IDB delete failed:', change.doc.id, e);
          }
        }
      } catch (e) {
        console.warn('[realtime] note change apply failed:', change.type, change.doc.id, e);
      }
    }

    if (changedCount > 0) _scheduleHomeRender();
  }, (err) => {
    console.warn('[realtime] notes listener error (will auto-reconnect):', err);
  });

  // Folders listener — same shape, simpler (no ghost class for folders).
  _foldersUnsubscribe = foldersRef.onSnapshot(async (snap) => {
    let changedCount = 0;
    for (const change of snap.docChanges()) {
      if (change.doc.metadata.hasPendingWrites) continue;
      try {
        if (change.type === 'added' || change.type === 'modified') {
          await saveFolder(change.doc.data());
          changedCount++;
        } else if (change.type === 'removed') {
          try {
            const conn = await openDB();
            await new Promise((res, rej) => {
              ...
              tx.objectStore('folders').delete(change.doc.id);
              tx.oncomplete = res;
              tx.onerror = e => rej(e.target.error);
            });
            changedCount++;
          } catch (e) {
            console.warn('[realtime] folder IDB delete failed:', change.doc.id, e);
          }
        }
      } catch (e) {
        console.warn('[realtime] folder change apply failed:', change.type, change.doc.id, e);
      }
    }
    if (changedCount > 0) _scheduleHomeRender();
  }, (err) => {
    console.warn('[realtime] folders listener error (will auto-reconnect):', err);
  });
}

let _notesInitialDelivered = false;

function stopRealtimeSync() {
  if (_notesUnsubscribe)   { try { _notesUnsubscribe(); }   catch (e) {} _notesUnsubscribe = null; }
  if (_foldersUnsubscribe) { try { _foldersUnsubscribe(); } catch (e) {} _foldersUnsubscribe = null; }
  if (_renderDebounceHandle) { clearTimeout(_renderDebounceHandle); _renderDebounceHandle = null; }
  _notesInitialDelivered = false;
}
