...
assert_contains public/js/note_creation.js "if (!pptFile && !txtFiles.some(s => s.file !== null)) return;" "U1-1a: 단일 모드 분석 게이트 — PPT 없이 녹취록만 있어도 통과"
assert_contains public/js/note_creation.js "녹취록 전용 모드 — PPT 없음" "U1-1b: PPT 없을 때 진행 메시지가 녹취록 전용 모드를 알림"

assert_contains public/js/pptx_parser.js "analyzeBtn.disabled = isRunning || (!pptFile && !hasRecordings);" "U1-2a: checkReady가 PPT 없이도 녹취록만으로 분석 버튼 활성화"
...
assert_contains public/index.html 'id="pptInput" accept=".pptx,.pdf,.docx"' "U1-4c: 단일 모드 pptInput accept에 .docx 포함"
