...
assert_matches public/index.html "transcripts_view\.js\?v=u17fix" "U7-7: transcripts_view 캐시버스트 갱신 (현재값 정확 핀 — 얼터네이션 금지, 구값 허용은 회귀 감지력 상실)"
...
assert_matches public/index.html "recorder\.js\?v=u17deixis" "U7e-3: recorder.js 캐시버스트 갱신 (현재값 정확 핀)"
